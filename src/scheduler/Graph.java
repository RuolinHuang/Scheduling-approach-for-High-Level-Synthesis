...
public class Graph implements Iterable<Node> {
	...
	private TreeMap<Integer, Node> numbered_nodes;
	
	public void add_numbering(Integer key, Node node)
	{
		numbered_nodes.put(key, node);
	}
	
	public Node get_node(int number)
	{
		return numbered_nodes.get(number);
	}
		
	public Graph() {
		nodes = new HashMap<Node, Node>();
		numbered_nodes = new TreeMap<Integer, Node>();
	}
	...
	public int critical_path(Node node)
	{
		...
		while(node.allSuccessors()!=null)
		{
			...
			if(map_successor.keySet().iterator().hasNext()==true)
			{
				...
				critical_path += successor.getDelay();	//longest path, smallest number, for the sorting in Tree
